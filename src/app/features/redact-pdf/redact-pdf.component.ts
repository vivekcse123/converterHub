...
@Component({
  ...
  template: `
        ...
        <div>
          ...
          <div class="grid grid-cols-6 gap-2 mb-2 items-center">
            ...
            <input type="number" [(ngModel)]="r.height" placeholder="H"
                   class="col-span-1 px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white text-xs" />
            <button (click)="removeRegion($index)" class="text-xs text-red-400 hover:text-red-600">✕</button>
          </div>
          }
          @if (!regions.length) {
          <p class="text-xs text-slate-400">Add at least one region to redact.</p>
          }
        </div>
    ...
    <app-tool-info-section toolId="redact-pdf" />
  `,
})
