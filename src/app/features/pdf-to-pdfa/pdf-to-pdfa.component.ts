...
@Component({
  selector: 'app-pdf-to-pdfa',
  standalone: true,
  imports: [AdBannerComponent, ToolInfoSectionComponent],
  template: `
    <div class="max-w-2xl mx-auto px-4 py-10">
      <h1 class="text-2xl font-bold text-slate-800 dark:text-white mb-1">PDF to PDF/A</h1>
      <p class="text-slate-500 text-sm mb-6">Convert a PDF to archival PDF/A standard for long-term storage.</p>
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 space-y-4">
        ...
        <div class="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl text-center">
          ...
          <button type="button" (click)="c.downloadResult(result()!)" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700">Download PDF/A</button>
        </div>
        }
      </div>
      <app-ad-banner slot="horizontal" class="mt-6" />
    </div>
    <app-tool-info-section toolId="pdf-to-pdfa" />
  `,
})
export class PdfToPdfaComponent {
  ...
  go(): void { if (!this.file()) return; this.result.set(null); this.c.pdfToPdfa(this.file()!).subscribe({ next: r => this.result.set(r), error: err => this.n.error(err?.error?.message || 'Conversion failed') }); }
}
