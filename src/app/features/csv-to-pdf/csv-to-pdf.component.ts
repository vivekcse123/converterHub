...
@Component({
  selector: 'app-csv-to-pdf',
  standalone: true,
  imports: [AdBannerComponent, ToolInfoSectionComponent],
  template: `
    <div class="max-w-2xl mx-auto px-4 py-10">
      <h1 class="text-2xl font-bold text-slate-800 dark:text-white mb-1">CSV to PDF</h1>
      <p class="text-slate-500 text-sm mb-6">Convert a CSV spreadsheet into a formatted PDF table.</p>
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 space-y-4">
        <div (click)="fi.click()" class="border-2 border-dashed border-slate-200 dark:border-slate-600 rounded-xl p-8 text-center cursor-pointer hover:border-indigo-400 transition-colors">
          ...
          <input #fi type="file" accept=".csv" class="hidden" (change)="onFile($event)" />
        </div>
        @if (c.isConverting()) {
        <div>
    ...
    <app-tool-info-section toolId="csv-to-pdf" />
  `,
})
export class CsvToPdfComponent {
  ...
  go(): void { if (!this.file()) return; this.result.set(null); this.c.csvToPdf(this.file()!).subscribe({ next: r => this.result.set(r), error: err => this.n.error(err?.error?.message || 'Conversion failed') }); }
}
