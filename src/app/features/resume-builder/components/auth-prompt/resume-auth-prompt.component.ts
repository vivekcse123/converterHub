    ...
    @if (gate.showPrompt()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" (click)="gate.dismiss()">
        <div class="card relative p-6 max-w-sm w-full space-y-4 text-center" (click)="$event.stopPropagation()">
          ...
          <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100">You're one step away!</h3>
          <p class="text-sm text-slate-500 dark:text-slate-400">
            Sign in to download your professionally designed resume and save your progress.
          </p>

          <div class="flex flex-col gap-2 pt-1">
            <button type="button" class="btn btn-primary w-full" (click)="gate.goToLogin()">
              Sign In
            </button>
            <button type="button" class="btn btn-secondary w-full" (click)="gate.goToSignup()">
              Create Account
            </button>
            <button type="button" class="btn btn-ghost w-full" (click)="gate.dismiss()">
              Continue Editing
            </button>
          </div>
        </div>
